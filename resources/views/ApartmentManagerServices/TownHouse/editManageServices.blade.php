...
<body>
    ...
    <div class="contus-container">
        <h2>Edit TownHouse Manage Service</h2>
    </div>
    <br /><br /><br/>
    <div class="row justify-content-center">
        <div class="col-lg-6 col-md-8 col-sm-10">
            <form method="post" action="/apartmentManager/TownHouse/updateManageServices?emailId={{$emailId}}">
                @csrf
                <div class="form-group row">
                    <label for="exampleInputEmail1" class="col-sm-4 col-form-label">TownHouse Email Id: {{$emailId}}</label><br/>
                </div><br/>
                <div class="form-group row">
                    <label for="exampleInputEmail1" class="col-sm-4 col-form-label">Enter Service Type:</label><br />
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="serviceType" value="{{$serviceType}}"><br/>
                        @if ($errors->has('serviceType'))
                    <span class="field-error">{{ $errors->first('serviceType') }}</span><br/>
                    @endif
                    </div>
                    
                </div><br/>
                <div class="form-group row">
                    <label for="exampleInputEmail1" class="col-sm-4 col-form-label">Enter Service Handler Name:</label><br />
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="serviceHandlerName" value="{{$serviceHandlerName}}"/><br />
                        @if ($errors->has('serviceHandlerName'))
                    <span class="field-error">{{ $errors->first('serviceHandlerName') }}</span><br/>
                    @endif   
                    </div>
                ...
                <div class="form-group row">
                    <div class="col-sm-12 text-center">
                        ...
                        <button type="cancel" onclick="window.location.href='{{ url('/apartmentManager/TownHouse/manageServices') }}'" class="btn btn-outline-secondary btn-lg">Cancel</button>&nbsp;
                    </div>
                </div>
            </form>
        </div>
