...
<html>
    ...
    <body>
        ...
        <div class="div">
            <a href="/buildingManager/createParking">Create &nbsp;<i class='fa fa-plus'></i></a>
        </div>
        <div class="container1">
            <table>
                ...
                <tr>
                    ...
                    <td>{{$Parking->apartmentType}}</td>
                    <td>{{$Parking->ownerName}}</td>
                    <td>{{$Parking->email}}</td>
                    <td>{{$Parking->slotId}}</td>
                    <td>{{$Parking->occupied}}</td>
                    <td>{{$Parking->managerName}}</td>
                    <td> &nbsp;<a href="/buildingManager/editParking?apartmentType={{$Parking->apartmentType}}&ownerName={{$Parking->ownerName}}&email={{$Parking->email}}&slotId={{$Parking->slotId}}&occupied={{$Parking->occupied}}&managerName={{$Parking->managerName}}"><i class='fa fa-edit' ></i></a>&nbsp;
                        <a href="/buildingManager/deleteParking?email={{$Parking->email}}"><i class='fa fa-trash-o'></i></a>
                    </td>
                </tr>
                @endforeach
            </table>
