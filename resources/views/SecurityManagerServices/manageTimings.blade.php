...
<body>
    ...
    <div class="div">
        <a href="/SecurityManagerServices/createManageTimings">Create &nbsp;<i class='fa fa-plus'></i></a>
    </div>
    <div class="container1">
        <table>
            ...
            @foreach($timings as $timings)
            <tr>
                <td><?php $counter=$counter+1; echo $counter ?></td>
                <td>{{$timings->name}}</td>
                <td>{{$timings->emailId}}</td>
                <td>{{$timings->location}}</td>
                <td>{{$timings->schedule}}</td>
                <td>{{$timings->shift}}</td>
                <td> &nbsp;<a href="/SecurityManagerServices/editManageTimings?name={{$timings->name}}&emailId={{$timings->emailId}}&location={{$timings->location}}&schedule={{$timings->schedule}}&shift={{$timings->shift}}"><i class='fa fa-edit' ></i></a>&nbsp;
                    <a href="/SecurityManagerServices/deleteManageTimings?emailId={{$timings->emailId}}"><i class='fa fa-trash-o'></i></a>
                </td>
            </tr>
            @endforeach
        </table>
