...
<body>
    ...
    <div class="container">
        <h2>Tennis Manager Staff List</h2>
    </div>
    <div class="div">
        <a href="/tennisManager/createTennisStaffList">Create &nbsp;<i class='fa fa-plus'></i></a>
    </div>
    <div class="container1">
        <table>
            ...
            @foreach($tennisStaffList as $staffList)
            <tr>
                <td><?php $counter=$counter+1; echo $counter ?></td>
                <td>{{$staffList->name}}</td>
                <td>{{$staffList->email}}</td>
                <td>{{$staffList->workerType}}</td>
                <td>{{$staffList->tennisName}}</td>
                <td>{{$staffList->schedule}}</td>
                <td> &nbsp;<a href="/tennisManager/editTennisStaffList?name={{$staffList->name}}&email={{$staffList->email}}&workerType={{$staffList->workerType}}&tennisName={{$staffList->tennisName}}&schedule={{$staffList->schedule}}"><i class='fa fa-edit' ></i></a>&nbsp;
                    <a href="/tennisManager/deleteTennisStaffList?email={{$staffList->email}}"><i class='fa fa-trash-o'></i></a>
                </td>
            </tr>
            @endforeach
        </table>
